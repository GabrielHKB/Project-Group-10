...
class ImatDataHandler extends ChangeNotifier {
  ...
  final List<Product> _baseSelection     = [];
  final List<Product> _filteredSelection = [];
  ...
  // New methods added for filter_bar
  ...
  List<Product> filterProducts({
    ...
    return _products.where((p) {
      final inRange  = p.price >= minPrice && p.price <= maxPrice;
      final ecoCheck = !ecoOnly || p.isEcological;
      return inRange && ecoCheck;
    }).toList();
  }
  // START OF ADDED METHODS
  /// Applies price+eco filter to the *current selection* by
  /// overwriting `selectProducts` and notifying listeners.
  void applyFilter({
    required double minPrice,
    required double maxPrice,
    bool ecoOnly = false,
  }) {
    final filtered = _baseSelection.where((p) {
      final inRange  = p.price >= minPrice && p.price <= maxPrice;
      final ecoCheck = !ecoOnly || p.isEcological;
      return inRange && ecoCheck;
    }).toList();
  ...
  /// DEBUG only: exactly like [filterByPriceAndEco] but logs each resulting product
  List<Product> debugFilterByPriceAndEco({
    required double minPrice,
    required double maxPrice,
    bool ecoOnly = false,
  }) {
    ...
    /// Print every product that made it through
    result.forEach((p) {
      debugPrint('DEBUG FILTER: ${p.name} → isEcological=${p.isEcological}');
    });
